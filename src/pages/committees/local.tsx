...
export default function LocalCommittees() {
  const states = [
    ...
    <div className="min-h-screen bg-background py-12">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          {states.map((state) => {
            const slug = state.toLowerCase().replace(/ /g, '-').replace(/[^a-z0-9-]/g, '') + '-committee';
            return (
              <Link
                key={state}
                to={`/committees/chat/${slug}`}
              ...
              >
                ...
                <div className="font-semibold text-patriot-blue">{state} Committee</div>
              </Link>
            );
          })}
        </div>
